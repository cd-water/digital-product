<template>
  ...
  >
    ...
    <el-table-column label="余额" prop="balance" align="center" width="150">
      <template #default="{ row }">
        <span style="color: var(--danger-color); font-size: var(--font-size-lg); font-weight: bold">
          ￥{{ row.balance }}
        </span>
      </template>
    </el-table-column>
  ...
  >
    <el-form ref="dialogRef" :model="dialogState.form" :rules="dialogState.rules" label-width="80px">
      ...
      <el-form-item v-if="dialogState.form.id" prop="balance" label="余额">
        <span style="color: var(--danger-color); font-size: var(--font-size-lg); font-weight: bold">
          ￥{{ dialogState.form.balance }}
        </span>
      </el-form-item>
    </el-form>
    <template #footer>
...
const handlePage = async () => {
  ...
  try {
    const res = await pageUserApi(tableState.value.searchForm, tableState.value.pageNum, tableState.value.pageSize)
    if (res.code === 200) {
...
const handleEdit = async (row) => {
  ...
  try {
    dialogState.value.title = '编辑用户'
    dialogRef.value?.resetFields?.()
    const res = await queryUserApi(row.id)
    if (res.code === 200) {
...
const handleRemoveBatch = () => {
  ...
  ElMessageBox.confirm('删除操作不可逆，是否继续？', 'Warning', {
    ...
    .then(async () => {
      try {
        const res = await removeBatchUserApi(tableState.value.ids)
        if (res.code === 200) {
          ElMessage.success('删除成功')
          handlePage()
        }
...
const handleSave = async () => {
  ...
  try {
    ...
    if (dialogState.value.form.id) {
      res = await editUserApi(dialogState.value.form)
    } else {
      res = await addUserApi(dialogState.value.form)
    }
    if (res.code === 200) {
