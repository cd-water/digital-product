  ...
  <el-card shadow="hover">
      ...
      <div class="main-msg">
        ...
        <div>
          ...
          <div class="role">
            ...
            <span v-else-if="profileMsg.role === 1">店铺商家</span>
    ...
    <el-descriptions title="我的个人资料" direction="vertical" border :column="4">
      ...
      <el-descriptions-item label="余额">
        <span class="price">￥{{ profileMsg.balance }}</span>
  ...
  >
    <el-form ref="dialogRef" :model="dialogState.form" :rules="dialogState.rules" label-width="80px">
      ...
      <el-form-item v-if="dialogState.form.id" prop="balance" label="余额">
        <span class="price">￥{{ dialogState.form.balance }}</span>
      </el-form-item>
    </el-form>
    <template #footer>
      <div>
...
import { queryUserApi, editUserApi, topupApi } from '@/api/customer/user'
...
const handleEdit = async () => {
  ...
  try {
    ...
    const res = await queryUserApi(authStore.authMsg.id)
    if (res.code === 200) {
...
const handleSave = async () => {
  ...
  try {
    ...
    const res = await editUserApi(dialogState.value.form)
    if (res.code === 200) {
