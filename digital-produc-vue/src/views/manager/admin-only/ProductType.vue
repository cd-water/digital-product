<template>
  ...
  >
    <template #empty>
      <el-empty description="暂无数码产品类型信息" />
    </template>
...
import {
  addProductTypeApi,
  removeProductTypeApi,
  removeBatchProductTypeApi,
  editProductTypeApi,
  queryProductTypeApi,
  pageProductTypeApi,
} from '@/api/manager/productType'
...
const handlePage = async () => {
  ...
  try {
    const res = await pageProductTypeApi(
...
const handleAdd = () => {
  dialogState.value.title = '新增数码产品类型'
...
const handleEdit = async (row) => {
  ...
  try {
    dialogState.value.title = '编辑数码产品类型'
    dialogRef.value?.resetFields?.()
    const res = await queryProductTypeApi(row.id)
    if (res.code === 200) {
...
const handleRemove = (id) => {
  ...
  ElMessageBox.confirm('删除操作不可逆，是否继续？', 'Warning', {
    ...
    .then(async () => {
      try {
        const res = await removeProductTypeApi(id)
        if (res.code === 200) {
...
const handleRemoveBatch = () => {
  ...
  ElMessageBox.confirm('删除操作不可逆，是否继续？', 'Warning', {
    ...
    .then(async () => {
      try {
        const res = await removeBatchProductTypeApi(tableState.value.ids)
        if (res.code === 200) {
          ElMessage.success('删除成功')
          handlePage()
        }
...
const handleSave = async () => {
  ...
  try {
    ...
    if (dialogState.value.form.id) {
      res = await editProductTypeApi(dialogState.value.form)
    } else {
      res = await addProductTypeApi(dialogState.value.form)
    }
    if (res.code === 200) {
